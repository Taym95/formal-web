...
| Linking a dfn anchor from a function that implements an algorithm's steps (e.g. `#event-handler-content-attributes` on the handler-compilation function) | Anchor to the algorithm being implemented (e.g. `#getting-the-current-value-of-the-event-handler`). The dfn is the *concept*, not the steps; when the algorithm is unnamed in the spec, anchor to its enclosing section (e.g. `#event-handler-attributes`) and carry the algorithm's steps in `// Step N:` comments |
