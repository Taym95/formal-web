...
fn collect_rendering_opportunity_trace_ids(
    ...
    for event in trace_events {
        match event.event.as_str() {
            "NoteRenderingOpportunity" | "FrameRendered" => {
                if let Some(webview_id) = event.event_args.first() {
                    webview_ids.insert(webview_id.clone());
                }
            }
            _ => {}
        }
    ...
    let mut sets = BTreeMap::new();
    sets.insert(String::from("TraceWebViews"), webview_ids);
    sets
}
...
fn render_rendering_opportunity_trace_module(
    ...
    let webviews = named_sets
        .get("TraceWebViews")
        .map(render_string_set)
        .unwrap_or_else(|| String::from("{}"));

    format!(
        "------------------------- MODULE {module_name} -------------------------\n\\* Generated by verification from the current NDJSON trace.\nTrace == {trace_entries}\n\nTraceWebViews == {webviews}\n\nTraceMaxRenderingOps == 2\n\n=============================================================================\n",
    )
}
...
common cause. "
        ...
        "NoteRenderingOpportunity" => {
            "The model tried to note a rendering opportunity for a webview. When \
the webview has no pending render, the opportunity is promoted directly to a \
pending render. When the webview already has a pending render, the opportunity \
is only counted. This step fails if the webview has reached MaxRenderingOps \
noted opportunities. "
        }
        "FrameRendered" => {
            "The model tried to complete a pending frame render for a webview. \
The webview must have a pending render in flight (pending > 0). If noted \
opportunities accumulated while it was busy, they are immediately re-promoted \
(opp resets, pending stays 1). Otherwise pending clears to 0. "
        }
        _ => {
