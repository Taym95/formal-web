...
fn collect_rendering_opportunity_trace_ids(
    ...
    // The frame universe: every frame id referenced by a rendering event or
    // created by a CreateFrame event (which is the navigation path's
    // graphics-side effect, traced for this spec too). The parent of each
    // frame travels in the CreateFrame event args, so no separate
    // hierarchy map is needed: the trace model grows the frame set with
    // each CreateFrame event.
    ...
    for event in trace_events {
        match event.event.as_str() {
            "CreateFrame" => {
                ...
                if let Some(parent_id) = event.event_args.get(1) {
                    frame_ids.insert(parent_id.clone());
                }
            }
...
fn render_trace_data_module_for_spec(
    ...
    match spec_name {
        ...
        "RenderingOpportunity" => {
            render_rendering_opportunity_trace_module(module_name, &trace_entries, named_sets)
        }
        _ => {
...
fn render_rendering_opportunity_trace_module(
    ...
    // references), so TraceNone and TraceFrame must precede any operator
    // that uses them.
    format!(
        "------------------------- MODULE {module_name} -------------------------\n\\* Generated by verification from the current NDJSON trace.\nTrace == {trace_entries}\n\nTraceFrame == {trace_frame}\n\nTraceNoParent == \"NOPARENT\"\n\nTraceBufferCount == 2\n\n=============================================================================\n",
    )
}
...
fn event_failure_context(event: &str) -> &'static str {
    match event {
        "CreateFrame" => {
            "The model tried to create a new frame (navigable) with an \
optional parent. The frame id must be new (not yet created), and the \
parent must be an existing frame or NoParent for a top-level traversable. \
This event is the navigation path's graphics-side effect, traced for \
the RenderingOpportunity spec from CreateNavigable (one argument) and \
CreateChildNavigable (child, parent). "
        }
        "CreateNavigable" => {
