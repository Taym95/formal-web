...
fn write_trace_data_module(
    ...
    let named_sets = collect_trace_ids_for_spec(&spec.name, &trace_events);
    let module_path = spec.trace_data_module_path(run_dir);
    let module_contents = render_trace_data_module_for_spec(
        &spec.name,
...
fn collect_navigation_trace_ids(trace_events: &[TraceEvent]) -> BTreeMap<String, BTreeSet<String>> {
    ...
    sets.insert(String::from("TraceNavIDs"), nav_ids);
    sets.insert(String::from("TraceNkIDs"), nk_ids);
    sets
}

fn collect_rendering_opportunity_trace_ids(
    trace_events: &[TraceEvent],
) -> BTreeMap<String, BTreeSet<String>> {
    ...
    let mut sets = BTreeMap::new();
    sets.insert(String::from("TraceFrame"), frame_ids);
    sets
}

fn render_trace_data_module_for_spec(
    spec_name: &str,
    module_name: &str,
    trace_events: &[TraceEvent],
    named_sets: &BTreeMap<String, BTreeSet<String>>,
) -> String {
    let trace_entries = if trace_events.is_empty() {
        String::from("<<>>")
    } else {
        let entries = if spec_name == "GPURendering" {
            trace_events
                .iter()
                .map(render_gpu_rendering_trace_event)
                .collect::<Vec<_>>()
                .join(",\n    ")
        } else {
            trace_events
    ...
    match spec_name {
        "Navigation" => render_navigation_trace_module(module_name, &trace_entries, named_sets),
        "RenderingOpportunity" => render_rendering_opportunity_trace_module(
            module_name,
            &trace_entries,
            named_sets,
            trace_events,
        ...
        "GPURendering" => {
            render_gpu_rendering_trace_module(module_name, &trace_entries, trace_events)
        }
        _ => {
            let mut result = format!(
                "------------------------- MODULE {module_name} -------------------------\n\\* Generated by verification from the current NDJSON trace.\nTrace == {trace_entries}\n"
            );
            for (set_name, values) in named_sets {
                result.push_str(&format!("\n{set_name} == {}\n", render_string_set(values)));
            }
            result.push_str(
                "\n=============================================================================\n",
            );
...
fn render_navigation_trace_module(
    module_name: &str,
    trace_entries: &str,
    named_sets: &BTreeMap<String, BTreeSet<String>>,
) -> String {
    let nav_ids = named_sets
        .get("TraceNavIDs")
        .map(render_string_set)
        .unwrap_or_else(|| String::from("{}"));
    let nk_ids = named_sets
        .get("TraceNkIDs")
        .map(render_string_set)
        .unwrap_or_else(|| String::from("{}"));
...
fn render_rendering_opportunity_trace_module(
    module_name: &str,
    trace_entries: &str,
    named_sets: &BTreeMap<String, BTreeSet<String>>,
    trace_events: &[TraceEvent],
) -> String {
    let trace_frame = named_sets
        .get("TraceFrame")
        .map(render_string_set)
        .unwrap_or_else(|| String::from("{}"));

    // Count NoteRenderingOpportunity events per frame to set MaxCounter
    // large enough that the per-frame counter never overflows. The model
    // uses integer counters as an abstraction of the code's set-based
    // pending_renders/queued_opps tracking; MaxCounter is bounded only
    // to prevent TLC state-space explosion for model checking. For trace
    // validation, we need enough headroom to cover all trace events.
    let mut frame_counts: std::collections::HashMap<&str, usize> = std::collections::HashMap::new();
    for event in trace_events {
        if event.event == "NoteRenderingOpportunity" {
            if let Some(frame_id) = event.event_args.first() {
                *frame_counts.entry(frame_id.as_str()).or_insert(0) += 1;
            }
        }
    }
    let max_counter = frame_counts.values().copied().max().unwrap_or(0) + 2;

    format!(
        "------------------------- MODULE {module_name} -------------------------\n\\* Generated by verification from the current NDJSON trace.\nTrace == {trace_entries}\n\nTraceFrame == {trace_frame}\n\nTraceNone == \"NONEVAL\"\n\nTraceMaxCounter == {max_counter}\n\n=============================================================================\n",
    )
}
...
/// Render a GPURendering trace event with typed arguments:
/// SurfaceFrameSent:   [webview(str), generation(int), size(str), region(int)]
/// SurfaceFrameReceived: [webview(str), generation(int), size(str)]
/// The generation and region are rendered as TLA+ integer literals so the
...
fn render_gpu_rendering_trace_event(event: &TraceEvent) -> String {
    let rendered_args = match event.event.as_str() {
        "SurfaceFrameSubmitted" | "SurfaceFrameSent" => vec![
            ...
            parse_trace_u64(event.event_args.get(3))
                .unwrap_or_default()
                .to_string(),
        ],
        ...
        _ => {
            return render_trace_event(event);
        }
    };
    format!(
        "[event |-> {}, event_args |-> <<{}>>]",
...
fn parse_trace_u64(value: Option<&String>) -> Option<u64> {
    value.and_then(|arg| arg.parse::<u64>().ok())
}

/// Render the GPURendering trace data module: the typed event sequence plus
/// the constant sets derived from the trace (webviews and sizes as strings,
/// generations and regions as integers).
fn render_gpu_rendering_trace_module(
    module_name: &str,
    trace_entries: &str,
    trace_events: &[TraceEvent],
) -> String {
    let mut webviews = BTreeSet::new();
    let mut sizes = BTreeSet::new();
    let mut generations = BTreeSet::new();
    for event in trace_events {
        if let Some(webview) = event.event_args.first() {
            webviews.insert(webview.clone());
        }
    ...
    // Keep the model satisfiable even for an empty trace: the trace spec only
    // consumes recorded events, so the constant sets just need to be nonempty.
    if generations.is_empty() {
        generations.insert(1);
    }
    if webviews.is_empty() {
        webviews.insert(String::from("w0"));
    }
    if sizes.is_empty() {
        sizes.insert(String::from("s0"));
    }

    let rendered_webviews = render_string_set(&webviews);
    let rendered_sizes = render_string_set(&sizes);
    let rendered_generations = generations
        .iter()
        .map(u64::to_string)
        .collect::<Vec<_>>()
        .join(", ");
