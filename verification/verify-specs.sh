...
# Generic verification pipeline: runs the browser with TLA+ tracing enabled
# and validates all recorded traces against their respective TLA+ trace specs.
# Supports multiple specs (Navigation, RenderingOpportunity, etc.).
# Add new specs by writing {Name}.tla, {Name}Trace.tla, and adding trace
# events via verification::tla_log!(..., -> "Name", "EventName", ...) in the code.
...
if [[ -n "${FORMAL_WEB_VERIFY_WORK_DIR:-}" ]]; then
    ...
    WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/formal-web-verify-specs.XXXXXX")"
    WORK_DIR_CREATED=1
fi
...
"$ROOT/target/release/formal-web-embedder" --verify webdriver --headless --port "$PORT" \
    >"$LOG_FILE" 2>&1 &
FORMAL_WEB_PID="$!"
...
for i in {1..100}; do
    if ! kill -0 "$FORMAL_WEB_PID" 2>/dev/null; then
        fail_with_log "formal-web exited during initial page load"
    fi
    sleep 0.1
done
...
# Collect and report verification results for all specs.
any_failed=0
spec_results="$(grep -E '^(CHECK|SKIP|NONE) ' "$LOG_FILE" || true)"
if [[ -z "$spec_results" ]]; then
    fail_with_log "verification output did not contain any spec results"
fi

echo ""
echo "=== TLA+ verification results ==="
echo "$spec_results"
echo ""

while IFS= read -r line; do
    if [[ "$line" == CHECK*"FAIL"* ]]; then
        any_failed=1
    fi
done <<< "$spec_results"

if [[ $any_failed -eq 1 ]]; then
    fail_with_log "one or more specifications failed verification"
fi

echo "All specifications verified successfully"
